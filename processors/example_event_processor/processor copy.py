...
def get_div_factor(coin_address, file_path='Coins.json'):
    ...
    url = f"https://fullnode.mainnet.aptoslabs.com/v1/accounts/{address}/resource/0x1::coin::CoinInfo%3C{address}::{module}::{coin_type}%3E"
    print(url)
    ...
    if response.status_code == 200:
        ...
        return 10 ** 6  # Return default division factor if API fails
...
# print(f"Protocols: THALASWAP={THALASWAP}, LIQUIDSWAP={LIQUIDSWAP}, PANCAKE={PANCAKE}")
...
class ExampleEventProcessor(TransactionsProcessor):
    ...
    def process_transactions(
        ...
        for transaction in transactions:
            ...
            aptos_price = self.price_cache.get('0x1::aptos_coin::AptosCoin')

            if aptos_price is None:
                try:
                    aptos_price = get_price("0x1::aptos_coin::AptosCoin", amount=100000000)  
                    self.price_cache.set('0x1::aptos_coin::AptosCoin', aptos_price)
                except Exception as e:
                    print(f"Error fetching APTOS price: {e}")
                    aptos_price = 8.87
            ...
            for event_index, event in enumerate(user_transaction.events):
                ...
                if "SwapEvent" in type:
                    try:
                        ...
                        div_factor1 = get_div_factor(coin1)
                        div_factor2 = get_div_factor(coin2)
                        print(div_factor1 , div_factor2)
                        decimal_x=None
                        ...
                        if account_address == LIQUIDSWAP:
                            ...
                            price_x = 1 if coin1 == USDC_ADDRESS else self.price_cache.get(coin1)
                            price_y = 1 if coin2 == USDC_ADDRESS else self.price_cache.get(coin2)

                            if price_x is None:
                                try:
                                    price_x = get_price(coin1,amount=div_factor1,provider=provider)
                                    self.price_cache.set(coin1, price_x)
                                except Exception as e:
                            ...
                            if price_y is None:
                                try:
                                    price_y = get_price(coin2,amount= div_factor2,provider=provider)
                                    self.price_cache.set(coin2, price_y)
                                except Exception as e:
                            ...
                            print(f"Price of token X: {price_x}, Price of token Y: {price_y}, Delta X: {delta_x}, Delta Y: {delta_y}, Slippage: {slippage:.2f}%")
                            ...
                            # Fetch current price for Coin X if it is not in the cache or is None
                            if price_x is None:
                                try:
                                    price_x = get_price(coin1,amount=1,provider=provider)
                                    self.price_cache.set(coin1, price_x)
                                except Exception as e:
                                    print(f"Error fetching price for {coin1}: {e}")
                                    price_x = 0  # Assign default price 0 if fetching fails

                            # Fetch current price for Coin Y if it is not in the cache or is None
                            if price_y is None:
                                try:
                                    price_y = get_price(coin2,amount=1,provider=provider)
                                    self.price_cache.set(coin2, price_y)
                                except Exception as e:
                                    print(f"Error fetching price for {coin2}: {e}")
                                    price_y = 0  # Assign default price 0 if fetching fails

                            # Ensure price_x and price_y are not None (fallback to 0 if they are)
                            price_x = price_x if price_x is not None else 0
                            price_y = price_y if price_y is not None else 0
                            ...
                            # Log or print the slippage with detailed price_x and price_y information
                            print(f"Price of token X: {price_x}, Price of token Y: {price_y}, Delta X: {delta_x}, Delta Y: {delta_y} Slippage: {slippage:.2f}%")
                            ...
                            # Fetch or set prices for tokens X and Y
                            price_x = 1 if coin1 == USDC_ADDRESS else self.price_cache.get(coin1)
                            price_y = 1 if coin2 == USDC_ADDRESS else self.price_cache.get(coin2)
                            ...
                            if price_x is None:
                                try:
                                    ...
                                    print(f"Error fetching price for {coin1}: {e}")
                                    price_x = 0  # Default to 0 if fetching fails
                            ...
                            if price_y is None:
                                try:
                                    ...
                                    print(f"Error fetching price for {coin2}: {e}")
                                    price_y = 0  # Default to 0 if fetching fails
...
# =============================================================================================================================
            ...
            for change in changes:
                ...
                if "0x190d44266241744264b964a37b8f09863167a12d3e70cda39376cfb4e3561e12::liquidity_pool::LiquidityPool" in change_type:
                    ...
                    if coin1 == USDC_ADDRESS:
                        price_x = 1
                        price_y = self.price_cache.get(coin2)
                        if price_y is None:
                            try:
                                price_y = get_price(coin2,amount=div_factor2,provider=provider)
                                self.price_cache.set(coin2, price_y)
                        ...
                        price_y = 1
                        price_x = self.price_cache.get(coin1)
                        if price_x is None:
                            try:
                                price_x = get_price(coin1,amount= div_factor1,provider=provider)
                                self.price_cache.set(coin1, price_x)
                    ...
                    if price_x and price_y:
                        tvl = coin_x_reserve * float(price_x) + coin_y_reserve * float(price_y)
                    else:
    ...
    def insert_to_db(self, parsed_objs: dict) -> None:
        try:
            ...
            columns = [
                'version', 'timestamp', 'pool_address', 'coin1', 'coin2', 'provider',
